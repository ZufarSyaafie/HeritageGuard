...
**Goal:** Build the complete HeritageGuard frontend MVP — 8 routes with real YOLOv12 API integration, Zustand state, recharts dashboard, Canvas bounding-box overlay, auto-generated PDF-ready report, and profile page.
...
const useScanStore = create((set) => ({
	...
	startScan: () =>
		set({ scanStatus: "uploading", errorMsg: null, detections: [] }),
...
const ROLES = [
	...
	{
		value: "pemerintah",
		label: "Instansi Pemerintah (BPK / Dinas Kebudayaan)",
	},
		...
		<form className="space-y-6" onSubmit={(e) => e.preventDefault()}>
			...
			<div className="flex items-start gap-3 p-4 bg-blue-50/50 rounded-2xl border border-blue-100/50">
				...
				<p className="text-[11px] text-blue-800 leading-relaxed font-medium">
					...
					<span className="font-bold underline">Kebijakan Privasi</span>{" "}
					HeritageGuard.
				</p>
			</div>
		...
		<div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
			<div className="mb-6">
				<h3 className="text-lg font-bold text-gray-900">
					Tren Deteksi Kerusakan
				</h3>
				<p className="text-xs text-gray-400 mt-1 font-medium">
					6 bulan terakhir — jumlah deteksi per kategori
				</p>
			</div>
			...
			<section className="bg-white rounded-3xl p-8 border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] space-y-6">
				...
				<button
					onClick={() =>
						...
						useScanStore.setState({ scanStatus: "idle" })
					}
		...
		<section className="bg-white rounded-3xl p-8 border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] space-y-6">
			...
			>
				{imagePreview ? (
					...
					<>
						<div
							className={`w-24 h-24 rounded-3xl flex items-center justify-center mb-8 transition-all duration-500 ${isDragging ? "bg-primary text-white scale-110 rotate-12" : "bg-blue-50 text-primary"}`}
						>
		...
		<div className="bg-white rounded-xl border border-gray-200 p-8 print:rounded-none print:border-0 print:p-0">
			...
			<div className="text-center mb-8">
				...
				<p className="text-sm text-gray-500 mt-1">
					Deteksi Kerusakan Cagar Budaya — HeritageGuard AI
				</p>
			</div>
...
const REKOMENDASI = {
	crack:
		"Diperlukan injeksi epoxy pada retakan struktural dalam waktu 48 jam. Lakukan pemantauan berkala setiap 7 hari untuk memastikan retakan tidak meluas.",
	spalling:
		"Pengelupasan beton memerlukan patching dengan mortar repair khusus heritage. Bersihkan area spalling, aplikasikan bonding agent, dan tambal dengan mortar sebelum dilakukan pengecatan ulang.",
	moisture:
		"Infiltrasi kelembapan memerlukan aplikasi waterproofing membrane pada permukaan eksterior. Identifikasi sumber kebocoran dan perbaiki sealant pada sambungan struktur.",
};
					...
					<table className="w-full text-sm">
						<thead className="bg-gray-50">
							<tr>
								{[
									...
									<th
										key={h}
										className="px-6 py-3 text-left text-[10px] font-black text-gray-400 uppercase tracking-wider"
									>
										{h}
									</th>
								))}
						...
						<tbody className="divide-y divide-gray-50">
							{detectedLabels.map((label) => {
								...
								const level =
									pct >= 90 ? "Kritis" : pct >= 70 ? "Menengah" : "Rendah";
								return (
									<tr key={label} className="hover:bg-gray-50/50">
										...
										<td className="px-6 py-4">
											<span
												className={`px-2.5 py-1 rounded-full text-[11px] font-bold ${c.bg} ${c.text}`}
											>
												{level}
											</span>
										</td>
								...
								<div key={label} className="space-y-2">
									<div className="flex justify-between items-end">
										...
										<span className={`text-xs font-bold ${c.text}`}>
											{pct >= 90 ? "Critical" : pct >= 70 ? "Moderate" : "Low"}
										</span>
									</div>
					...
					<div className="space-y-4">
						{detectedLabels.map((label) => {
							...
							return (
								<div
									key={label}
									className={`p-5 rounded-xl ${c.bg} border border-opacity-20`}
								>
									<p
										className={`text-xs font-black uppercase tracking-widest mb-2 capitalize ${c.text}`}
									>
										{label}
									</p>
									<p className="text-sm text-gray-700 leading-relaxed">
										{REKOMENDASI[label] ??
											"Lakukan evaluasi lebih lanjut oleh insinyur konservasi."}
									</p>
								</div>
							);
						})}
